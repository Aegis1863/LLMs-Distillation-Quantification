...
from easyjailbreak.metrics.Evaluator import EvaluatorFactorJudge
...
class GPTFuzzerIdentity(AttackerBase):
    """
    ...
    """
    def __init__(self, attack_model, target_model, eval_model, factor_judge=None, fact=None, start_mark='<judge>', end_mark='</judge>', jailbreak_datasets:JailbreakDataset = None, energy:int = 1,  max_query: int = 100, max_jailbreak: int = 100, max_reject: int = 100, max_iteration: int = 100, seeds_num = 76, template_file = None):
        """
        ...
        self.evaluator = EvaluatorFactorJudge(self.eval_model, factor_judge, fact, start_mark, end_mark)
        ...
        try:
            while not self.is_stop():
                ...
                for mutator_instance in mutated_results:
                    ...
                    for query_instance in tqdm(self.Questions, mininterval=10, ncols=160):
