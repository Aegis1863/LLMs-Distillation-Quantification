...
for refer_prompt in ['GPT']:
    for eval_api in [gpt_4omini_custom_api]:
        for target_api in [target_custom_api]:
            ...
            dataset_name = f'identity_jailbreak/data/{refer_prompt}_identity.csv'
            ...
            attacker.attack()
            attacker.log()
            attacker.jailbreak_datasets.save_to_jsonl(f'identity_jailbreak/data/attack_result/{judge_name}_{refer_prompt}_{target.name}.jsonl')

            # ----------------------------
            # Organize data

            df = pd.read_json(f'identity_jailbreak/data/attack_result/{judge_name}_{refer_prompt}_{target.name}.jsonl', orient='records', lines=True)
            score = round(len(df[df['eval_results'] == True])/len(df), 2)

            print('Target_name:', target.name)
            print('Score:', score)

            os.mkdir(f'identity_jailbreak/data/loose_score/') if not os.path.exists(f'identity_jailbreak/data/loose_score/') else None

            df_only_break = df[df['eval_results'] == True]
            df_only_break.to_csv(f'identity_jailbreak/data/loose_score/{refer_prompt}_{target.name}_{score}.csv', index=False)
